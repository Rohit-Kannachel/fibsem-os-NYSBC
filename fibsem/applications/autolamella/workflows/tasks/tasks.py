...
class SetupLamellaTaskConfig(AutoLamellaTaskConfig):
    ...
    use_area_alignment: bool = field(
        default=False,
        metadata={"help": "Whether to use area alignment (vs Fiducial) for alignment",
                  "label": "Use Area Alignment"},
    )
    use_ml_model: bool = field(
        default=False,
        metadata={"help": "Whether to use an ML Model to assist with Alignment",
                  "label": "Use ML Model"},
    )
...
class MillUndercutTask(AutoLamellaTask):
    ...
    def _run(self) -> None:
        ...
        for i, undercut_milling_angle in enumerate(undercut_milling_angles):
            ...
            self._refresh_view(image_settings=image_settings,field_of_view=self.config.milling[UNDERCUT_KEY].field_of_view)
    ...
    def _run(self) -> None:
        ...
        if self.config.orientation == "FIB":
            ...
            if has_rotated:
                ...
                self._align_with_ml_locally(image_settings=image_settings,milling_key=FIDUCIAL_KEY)
        ...
        elif self.config.use_area_alignment:
        ...
        if self.config.use_fiducial or self.config.use_area_alignment:
